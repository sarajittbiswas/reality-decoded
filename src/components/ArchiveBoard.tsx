    ...
    <div className="w-full relative z-10">
      ...
      <div className="flex justify-center mb-16">
        <div className="relative flex items-center bg-[#050505] border border-white/10 rounded-full p-1.5 shadow-[inset_0_4px_20px_rgba(0,0,0,0.5)] w-full max-w-md">
          ...
          ></div>
          <button onClick={() => setActiveTab('videos')} className={`relative flex-1 py-3 text-xs md:text-sm font-mono font-bold uppercase tracking-widest rounded-full transition-colors duration-300 z-10 ${activeTab === 'videos' ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}>
            Video Database
          </button>
          <button onClick={() => setActiveTab('blogs')} className={`relative flex-1 py-3 text-xs md:text-sm font-mono font-bold uppercase tracking-widest rounded-full transition-colors duration-300 z-10 ${activeTab === 'blogs' ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}>
            Decrypted Files
          </button>
        </div>
      </div>
        ...
        {currentData.map((item, i) => {
          ...
          return (
            <Link href={targetUrl} key={item.id} className="relative block group hover:z-50 animate-[fade-in-up_0.5s_ease-out_both]" style={{ animationDelay: `${i * 0.05}s` }}>
              ...
              <article className="relative bg-[#111111]/80 backdrop-blur-xl rounded-2xl border border-white/5 transition-all duration-500 hover:-translate-y-2 hover:border-purple-500/50 hover:shadow-[0_15px_40px_rgba(168,85,247,0.2)] h-full flex flex-col">
                
                <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/10 group-hover:via-purple-500/50 to-transparent transition-colors duration-500 z-20 rounded-t-2xl"></div>

                <div className="relative aspect-video w-full overflow-hidden rounded-t-2xl bg-black border-b border-white/5">
                ...
                <div className="p-5 flex-grow flex flex-col z-20 bg-[#111]/90 rounded-b-2xl">
                  ...
                  <p className="text-sm text-gray-500 line-clamp-2 mt-auto leading-relaxed mb-4">
                    ...
                    <div className="mt-auto border-t border-white/5 pt-3 flex items-center justify-between">
                      {item.agent ? (
                        ...
                        <AuthorHoverCard agent={item.agent}>
                          <div className="flex items-center gap-2 group/authboard">
                            <img src={item.agent.avatar || '/default-cover.png'} className="w-5 h-5 rounded-full border border-purple-500/30 group-hover/authboard:border-purple-400 transition-colors" />
                            <span className="text-[10px] text-gray-400 group-hover/authboard:text-white uppercase tracking-widest font-bold transition-colors">{item.agent.name.split(' ')[0]}</span>
                          </div>
                        </AuthorHoverCard>
                      ) : (
                        <span className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">By {item.author}</span>
                      )}
                    </div>
                  )}
                </div>
