...
export default async function Home() {
  ...
  return (
    <main className="w-full bg-[#0a0a0a] text-white overflow-hidden">
      ...
      {/* 
        ...
        This pure CSS block handles the fade-in-up effect as you scroll down the page.
      */}
      <style>{`
        ...
        .scroll-reveal {
          ...
          /* Modern scroll-timeline for browsers that support it */
          animation-timeline: view();
          animation-range: entry 5% cover 25%;
        }
        /* Fallback for the top section so it animates immediately on load */
        .load-reveal {
      ...
      {/* 1. HERO SECTION */}
      {/* Added pt-32 to push it out from under the fixed navbar */}
  ...
  <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-purple-500 drop-shadow-[0_0_15px_rgba(168,85,247,0.3)]">
    The Truth
          ...
          {/* Right Split Image Block - NOW WITH GLOW AND HOVER ANIMATIONS */}
          <div className="
          ...
          ">
            ...
            {/* Diagonal Neon Divider Line to make the split look intentional and sharp */}
           {/* <div 
              className="absolute top-[-10%] bottom-[-10%] w-1 bg-gradient-to-b from-transparent via-purple-500 to-transparent opacity-50 group-hover:opacity-100 transition-opacity duration-500 z-30" 
              style={{ left: '45%', transform: 'rotate(15deg)' }}
            ></div> */}
          </div>
        </div>
      </section>
            ...
            {results.map((video: any) => (
              <Link href={`/watch/${video.id}`} key={video.id} className="block group">
                <article className="
                  ...
                  bg-[#111111]/80 backdrop-blur-xl border border-white/5 
                  transition-all duration-500 ease-out
                  group-hover:-translate-y-2 group-hover:bg-[#161616]/90 group-hover:border-purple-500/30
                  group-hover:shadow-[0_10px_40px_rgba(168,85,247,0.2)]
                ">
                  ...
                  <div className="aspect-video bg-black overflow-hidden relative border-b border-white/5">
                    <img src="https://images.unsplash.com/photo-1611162617474-5b21e879e113?q=80&w=1000&auto=format&fit=crop" alt={video.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 opacity-80 group-hover:opacity-100" />
                    ...
                    <div className="absolute inset-0 bg-purple-900/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center pl-1 shadow-[0_0_20px_rgba(255,255,255,0.5)] group-hover:scale-110 transition-transform duration-300">
                  ...
                  <div className="p-6 flex-grow">
                    <h3 className="font-bold text-xl mb-3 text-white group-hover:text-purple-400 transition-colors duration-300">
                      {video.title}
                    </h3>
